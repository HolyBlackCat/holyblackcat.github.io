...
Flags for different kinds of module units|`-xc++-module` for importable units (optional for `.cppm`)<br/>`-xc++` otherwise (optional for `.cpp`)|No|`/interface` for interface units (optional for `.ixx`)<br/>`/internalPartition` for implementation partitions
