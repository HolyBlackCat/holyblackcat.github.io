...
Writes header deps to `a.d`, based on `-MF ...`. Change to `-MF -` to print to `stdout` (omitting prints to the filename given as `-o ...`, though in this command we are already arranging to use that filename for a different purpose).
